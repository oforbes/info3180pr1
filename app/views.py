...
def profile():
    ...
    if request.method == 'POST':
        ...
        user = User(uploadedfilename,form.username.data,form.firstname.data,form.lastname.data,form.age.data,form.sex.data,form.userbio.data,datetime.now())
        db.session.add(user)
        db.session.commit()
...
def selectedprofile(userid):
  ...
  if request.method == 'POST' or ('Content-Type' in request.headers and request.headers['Content-Type'] == 'application/json'):
    return jsonify(id=user.id, image=image,username=user.username, sex=user.usersex, age=user.userage, userbio=user.userbio,addedon=user.useraddon)
  else:
    user = {'id':user.id,'image':image, 'username':user.username,'fname':user.userfname, 'lname':user.userlname,'age':user.userage, 'sex':user.usersex, 'userbio':user.userbio,'addon':timeinfo(user.useraddon)}
